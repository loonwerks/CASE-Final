...
object Schedulers {
  ...
  val PFC_Sys_Impl_Instance_processor_pfc_timingProperties: ProcessorTimingProperties = ProcessorTimingProperties(
    clockPeriod = Some(2),
    framePeriod = Some(1000),
    maxDomain = Some(5),
    slotTime = None())

  val PFC_Sys_Impl_Instance_producer_producer_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = Some((10, 10)),
    domain = Some(2))

  val PFC_Sys_Impl_Instance_filter_filter_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = Some((10, 10)),
    domain = Some(3))

  val PFC_Sys_Impl_Instance_consumer_consumer_timingProperties: ThreadTimingProperties = ThreadTimingProperties(
    computeExecutionTime = Some((10, 10)),
    domain = Some(4))
  ...
  val staticSchedule: DScheduleSpec = DScheduleSpec(0, 0, DSchedule(ISZ(
    Slot(Arch.PFC_Sys_Impl_Instance_producer_producer.id, maxExecutionTime),
    Slot(Arch.PFC_Sys_Impl_Instance_filter_filter.id, maxExecutionTime),
    Slot(Arch.PFC_Sys_Impl_Instance_consumer_consumer.id, maxExecutionTime)
  )))
