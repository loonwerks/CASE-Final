...
The following assumes a case-env is used (see [https://github.com/sireum/case-env](https://github.com/sireum/case-env))

### Install CakeML

Run ``$HOME/CASE/seL4-CAmkES-L4v-dockerfiles/scripts/cakeml.sh``.  Note that installing HOL takes a long time and is optional

Once installed, make sure the 64bit version is available from the command line.  E.g. add the following symlink in a directory available in your ``cake -> /usr/local/bin/cake-x64-64/cake``
...
Entering pre-init of CASE_AttestationGate_thr_Impl_am_gate_CASE_AttestationGate
